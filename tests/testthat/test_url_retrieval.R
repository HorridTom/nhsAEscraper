...
test_that("getAEdata_urls_monthly returns urls with correct anatomy",{
  ...
  url_stem_presence <- str_detect(urls_ae, url_pattern)
  filename_strings <- str_match(urls_ae, filename_pattern)[,1]
  filename_strings <- str_match(filename_strings, '^/(.*)$')[,2]
  dates <- str_match(filename_strings, date_pattern)[,2]
  dates_valid <- !is.na(as.yearmon(dates, "%B-%Y"))
